      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 px-5 gap-5 pb-10">
        ...
        {posts.map((post) => (
          <ClientSideRoute key={post._id} route={`/post/${post.slug.current}`}>
            <div
              key={post._id}
              className="flex rounded-lg pb-7 flex-col group cursor-pointer hover:border-solid hover:border-2 hover:border-sky-600"
            >
                ...
                <div className="absolute bottom-0 w-full bg-gray-950 backdrop-blur-lg rounded drop-shadow-lg text-white p-5 flex justify-between">
                  ...
                  <div className="flex flex-col md:flex-row gap-y-2 md:gap-x-2 items-center">
                    {post.categories.map((category) => (
                      <div
                        key={category._id}
                        className="bg-sky-900 text-center text-white px-3 py-1 rounded-full text-sm font-semibold]"
                      >
                        <p>{category.title}</p>
                      </div>
                    ))}
